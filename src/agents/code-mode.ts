...
export function createCodeModeTools(ctx: CodeModeToolContext): AnyAgentTool[] {
  ...
  const execTool = markCodeModeControlTool({
    ...
    parameters: Type.Object({
      ...
      code: Type.String({
        description:
          'OpenClaw Code Mode requires JS/TS; no Python, shell, `require`, or `import`. Emit output with `text(value)` or `json(value)`. Use `return value`; a trailing expression yields `null`. Call enabled async globals directly. Compose deterministic dependent work in one cell; use Promise.all only for independent calls. Declared output fields may feed later calls in the same program. Observe again only for genuinely ambiguous output or changed authority, and never replay an uncertain mutation. Discover omitted tools with `catalog.search(query)`: `const [tool] = await catalog.search("..."); return await tool({...});`.',
      }),
